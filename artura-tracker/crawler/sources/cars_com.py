...
LABEL = "Cars.com"
KIND = "marketplace"
URL = ("https://www.cars.com/shopping/results/?stock_type=all&makes[]={make_slug}&models[]={make_slug}-{model_slug}"
...
def fetch(client, ctx: Ctx):
    ...
    out = []
    for page in (1, 2, 3):
        url = ctx.url(URL, zip=config.SEARCH_ZIP, page=page)
...
def _parse_rendered(html: str, base: str) -> list[Listing]:
    ...
    for a in soup.select("a[href*='/vehicledetail/']"):
        ...
        for _ in range(6):
            if "$" in text and is_target(text):
                break
            node = node.parent
            if node is None:
                break
            text = node.get_text("\n", strip=True)
        if "$" not in text or not is_target(text):
            continue
        if hasattr(node, "select") and len({x.get("href", "").split("?")[0] for x in node.select("a[href*='/vehicledetail/']")}) > 1:
            continue
        seen.add(href)
        lines = [t for t in text.split("\n") if t.strip()]
        title = next((t for t in lines if re.search(r"20\d\d.*" + current_target()["alias_re"], t, re.I)), current_target()["label"])
        ...
        dealer = next((t for t in lines if re.search(r"McLaren|Motors|Auto|Cars|Imports|Group|Dealer|Porsche|Ferrari|Lamborghini|Mercedes|BMW|Audi|Lexus|Bentley|Maserati|Aston", t) and "$" not in t and not is_target(t)), None)
