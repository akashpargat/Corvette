...
from ..models import Listing, parse_mileage, parse_price, vin_matches, current_target
...
API = HOST + "/gateway/api/purchasefunnel/v1/srp/inventory?make={make_slug}&model={model_slug}&radius=6000&zip={zip}&pagesize=100&pagenum={page}&sortby=price:asc&inventorytype=used,cpo,new"
PAGE_T = HOST + "/used-{make_slug}-{model_slug}/"
...
def fetch(client, ctx: Ctx):
    ...
    for page in (1, 2):
        ...
        if not data:
            ...
            break
        rows = list(walk(data, lambda d: isinstance(d.get("vin"), str) and vin_matches(d["vin"])))
...
def _row(r: dict) -> Listing:
    ...
    l = Listing(source=NAME, source_name=LABEL, url=HOST + (r.get("vdpLink") or f"/inventory/{r['vin']}.html"),
                title=f"{sv.get('year','')} {current_target()['label']} {sv.get('trim','') or ''}".strip(), vin=r["vin"],
